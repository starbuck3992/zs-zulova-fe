<template>
    ...
    <div v-if="mainPage" class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2 xl:grid-cols-3 px-5">
      <template v-for="post in posts" :key="post.id">
        <NuxtLink v-if="post.slug" :to="`/articles/${post.slug}`" class="flex flex-col items-start  shadow rounded-2xl">
            <div class="relative w-full">
                <img :src="config.public.apiBase + 'assets/' + post.thumbnail" alt=""
