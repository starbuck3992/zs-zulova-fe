<template>
  <div>
    ...
    >
      ...
      <div
        v-if="
          pageData && pageData[0]?.gallery && pageData[0]?.gallery?.length > 0
        "
        class="flex justify-content-center mt-10 w-full"
      >
        <Galleria
          v-model:activeIndex="activeIndex"
          v-model:visible="displayCustom"
          :value="pageData[0].gallery"
        ...
        <ul
          v-if="pageData && pageData[0].gallery"
          role="list"
          class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8"
        >
          <li
            v-for="(image, index) of pageData[0].gallery"
            :key="image.id"
            class="relative"
          >
            <div
